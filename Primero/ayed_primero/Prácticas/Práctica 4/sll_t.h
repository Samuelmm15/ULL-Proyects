...
template <class T> class sll_t {
 public:
  ...
  // getters
  sll_node_t<T>* get_head(void) const { return head_; }; /// para obtener la cabeza de la lista
  
  bool empty(void) const; /// para comprobar si la lista está vacía

  // operaciones
  void push_front(sll_node_t<T>*); /// para insertar valores al comienzo de la lista
  sll_node_t<T>* pop_front(void); /// para borrar valores del comienzo de la lista
 
  void insert_after(sll_node_t<T>*, sll_node_t<T>*); /// para insertat un valor después
  sll_node_t<T>* erase_after(sll_node_t<T>*); /// para borrar un valor después

  sll_node_t<T>* search(const T&) const; /// para buscar algún valor

  // E/S
  std::ostream& write(std::ostream& = std::cout) const;

 private:
  sll_node_t<T>* head_; /// puntero que apunta a la cabeza de la lista
};
