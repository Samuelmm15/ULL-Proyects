...
private:
  bool is_ok_(const int, const int) const; /// funcion para comprobar si una celda que esta situada en la fila i y la columna j es adecuada para pasar por ella
  bool solve_(const int, const int); 
};

istream& operator>>(istream&, maze_t&); /// sobrecarga de los operadores << y >>
