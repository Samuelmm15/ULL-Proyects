...
class sparse_vector_t { /// clase sparse_vector_t
 public:
  // constructores
  sparse_vector_t(const int = 0); /// constructor de la clase
  ...
  sparse_vector_t& operator=(const sparse_vector_t&); /// sobrecarga del operador
  ...
  // getters-setters
  pair_double_t& at(const int); /// sobrecarga de los operadores
...
bool IsNotZero(const double val, const double eps = EPS) { /// función para establecer que no se trata de cero, si no es cero retorna false, si es cero retorna true
  return fabs(val) > eps;
}

sparse_vector_t::sparse_vector_t(const int n) : pv_(n), nz_(0), n_(n) {} /// constructor
