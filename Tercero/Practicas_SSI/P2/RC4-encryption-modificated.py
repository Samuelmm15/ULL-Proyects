...
def Menu():
    ...
    w_value = int(input("Finalmente introduzca el valor w para poder aplicar de manera correcta el algoritmo de Spritz: "))
    print()
    print(f"La clave introducida es {key_seed}")
    print(f"El mensaje a cifrar es {original_message}")
    
    RC4_encryption(key_seed, original_message, w_value)

# Fuction that make the RC4 encryption
def RC4_encryption(key_seed, original_message, w_value):
    ...
    # Start with the second part of the encrypt secuence SPRITZ
    k = 0
    byte = 1
    i = 0
    j = 0
    m = 0 # This is the k value
    t = 0 # This is the z value
    
    while (k != len(original_message)):
        
        # i = (i + 1) % 256 # This is the PRGA algorithm
        # j = (j + S_array[i]) % 256
        ...
        # SPRITZ algorithm
        i = (i + w_value) % 256
        j = (m + S_array[j + S_array[i]]) % 256
        m = (i + m + S_array[j]) % 256
        auxiliary = S_array[i] # At this point produces the swap operation
        S_array[i] = S_array[j]
        S_array[j] = auxiliary
        t = S_array[j + S_array[i + S_array[t + m]]]
