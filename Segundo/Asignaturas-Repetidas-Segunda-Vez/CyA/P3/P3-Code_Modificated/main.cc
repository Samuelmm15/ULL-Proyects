...
int main(int argc, char *argv[]) {
  ...
  if ((argc == 2) && (option != "-h") && (option != "--help")) { /// If the introduced option is different of --help or -h.
    ...
    bool operationComprobationFlag = false;
    ...
    for (int i = 0; i < fileContent.size(); i++) {
      ...
      if (dividedChains[0][0] == 'L') {
        operationComprobationFlag = true;
        std::string auxiliaryLineResult;
        for (int j = 0; j < dividedChains.size(); j++) {
          std::string auxiliary = dividedChains[j];
          for (int k = 0; k < auxiliary.size(); k++) {
            auxiliaryLineResult.push_back(auxiliary[k]);
          }
        }
        auxiliaryLineResult.push_back('+');
        Language auxiliaryLanguage;
        auxiliaryLanguage.ReversePolishNotation(auxiliaryLineResult, languageVector);
        Language languageToIntroduce;
        languageToIntroduce.setLanguage(auxiliaryLanguage.getLanguage());
        languageVector.push_back(languageToIntroduce);
        languageVector[5].LanguagePrint();
      }
      if ((dividedChains[0] != "-1") && (operationComprobationFlag != true)) { /// To obtain the alphabet of the Language.
      ...
      operationComprobationFlag = false;
    }
