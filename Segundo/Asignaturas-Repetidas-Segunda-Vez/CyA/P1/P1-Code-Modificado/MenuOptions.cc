...
void MenuOptions::ChainsLong(std::string outputFileName, int flag, Alphabet newAlphabet, Chain newChain) {
  ...
  if (flag == 0) {
    ...
    if (!outputFile.is_open()) {
      ...
      std::vector<std::string> alphabetAuxiliary = newAlphabet.getAlphabet();
      outputFile << "{ ";
      for (int i = 0; i < alphabetAuxiliary.size(); i++) {
        if (i != (alphabetAuxiliary.size() - 1)) {
 ...
 */
void MenuOptions::InverseChains(std::string outputFileName, int flag, Alphabet newAlphabet, Chain newChain) {
  std::fstream outputFile;
  if (flag == 0) {
 ...
 * @param newChain is the object of the introduced chain.
 */
void MenuOptions::PrefixesChains(std::string outputFileName, int flag, Alphabet newAlphabet, Chain newChain) {
  std::fstream outputFile;
  if (flag == 0) {
    outputFile.open(outputFileName, std::ios_base::out);
    if (!outputFile.is_open()) {
 ...
 */
void MenuOptions::SufixesChains(std::string outputFileName, int flag, Alphabet newAlphabet, Chain newChain) {
  std::fstream outputFile;
  if (flag == 0) {
    outputFile.open(outputFileName, std::ios_base::out);
    if (!outputFile.is_open()) {
 ...
 * @param newChain is the object of the introduced chains.
 */
void MenuOptions::Substrings(std::string outputFileName, int flag, Alphabet newAlphabet, Chain newChain) {
  std::fstream outputFile;
  if (flag == 0) {
    outputFile.open(outputFileName, std::ios_base::out);
    if (!outputFile.is_open()) {
