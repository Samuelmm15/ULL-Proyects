...
void menu(int &option) { 
    ...
    cout << "3. Introduzca 3 si quiere realizar la ejecución del programa modificado (inserción de 5000 valores en un rango entre 0 y 1000000)." << endl;
    cout << "4. Introduzca 4 si desea contar la cantidad de valores que existen en cada celda de la Tabla de Hash." << endl;
    cout << "5. Introduzca 5 si desea salir del menú y finalizar la ejecución del programa." << endl;
    ...
    do {
        ...
        switch(option) {
            ...
            case 3 :
                int position = 0;
                if (H.vDates_created == 0) {
                    cout << "La tabla hash no ha sido creada." << endl;
                    cout << "Seleccione la opción 2 para crearla." << endl;
                } else if (H.vDates_created == 1) {
                    int range = 1000000; 
                    int insert_value = 0; /// The key that it's gonna to insert
                    int counter = 0;
                    do {
                        insert_value = rand() % range; /// Insert random keys
                        position = P(insert_value);
                        H.Insert_Key_Hash_Table(insert_value, L, position);
                        counter++;
                    } while (counter != 5000);
                    H.Hash_Table_Counter(L);
                }
            break;
        }
    } while (option != 5);
