...
std::vector<std::string> FileOperations::ChainDivision(std::string line) {
  ...
  bool comprobationFlag = false;
  bool languageOperationFlag = false;
  bool access = false;
  for (int i = 0; i < line.size(); i++) {
    if (line[i] == '=') {
      comprobationFlag = true;
    ...
    if (comprobationFlag == true) {
      /// En este punto se comprueba si el lenguaje introducido se trata del resultado de una propia operación
      if ((line[i] == 'L')) {
        languageOperationFlag = true;
      }
      if (languageOperationFlag == true) {
        if ((line[i] != '\n') && (line[i] != ' ')) {
          auxiliary.push_back(line[i]);
          access = true;
        } else {
