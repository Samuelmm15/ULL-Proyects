...
class Hash_Table { 
    ...
    int getnDates();
    /**
     * @brief The fuction that counts the number of keys that exists
     * 
     * @return int 
     */
    void Hash_Table_Counter(List_Hash<Key> L);
...
template<class Key> 
void Hash_Table<Key>::Hash_Table_Counter(List_Hash<Key> L) {
    int celd_counter = 0;
    int result = 0;
    for (int i = 0; i < nDates; i++) {
        celd_counter++; /// To count the number that is in the vector table
        result = L.List_Hash_Counter();
        result = result + celd_counter;
        cout << "Para la celda número " << i << " existen " << result << " claves" << endl;
        celd_counter = 0;
        result = 0;
    }
};